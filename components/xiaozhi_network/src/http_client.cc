        ...
        switch (parse_state_) {
            ...
            case ParseState::HEADERS: {
                ...
                if (line.empty()) {
                    ...
                    xEventGroupSetBits(event_group_handle_, HTTP_CLIENT_EVENT_HEADERS_RECEIVED);
                } else {
            ...
            case ParseState::CHUNK_TRAILER: {
                ...
                if (line.empty()) {
                    ...
                    eof_ = true;
                    xEventGroupSetBits(event_group_handle_, HTTP_CLIENT_EVENT_COMPLETE);
                }
                // 忽略 trailer 头部
                break;
            }
...
void HttpClient::SetError() {
    ...
    xEventGroupSetBits(event_group_handle_, HTTP_CLIENT_EVENT_ERROR);
}
...
int HttpClient::GetStatusCode() {
    if (!headers_received_) {
        ...
        auto bits = xEventGroupWaitBits(event_group_handle_,
                                        HTTP_CLIENT_EVENT_HEADERS_RECEIVED | HTTP_CLIENT_EVENT_ERROR,
                                        pdFALSE, pdFALSE,
                                        pdMS_TO_TICKS(timeout_ms_));

        if (bits & HTTP_CLIENT_EVENT_ERROR) {
            return -1;
        }
        if (!(bits & HTTP_CLIENT_EVENT_HEADERS_RECEIVED)) {
...
void HttpClient::ResetRequestState() {
    ...
    xEventGroupClearBits(event_group_handle_,
                         HTTP_CLIENT_EVENT_HEADERS_RECEIVED |
                         HTTP_CLIENT_EVENT_BODY_RECEIVED |
                         HTTP_CLIENT_EVENT_ERROR |
                         HTTP_CLIENT_EVENT_COMPLETE);
}
