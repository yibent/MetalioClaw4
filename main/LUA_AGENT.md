...
设备进入聊天主界面后，会与后端建立 WebSocket，上报设备信息，然后等待服务端下发 Lua 脚本。设备用本地 `lua_runtime` 执行脚本，调用全局函数 `main`（可配置），把返回值以 JSON 发回，并把脚本输出显示在聊天里。脚本如果创建了 UI，会临时盖在聊天之上，结束后回到聊天。`ui.load()` 期间触摸只交给 Lua：主界面顶层的对话点击热区 / 状态栏不会再抢走事件。
